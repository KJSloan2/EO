...
############################### GeoTiff Resampling ###################################
######################################################################################
years = list(logJson["tiles"].keys())
for tileId in logJson["tiles"][str(year_start)]:
	for year in years:
		fName = str(locationKey+"_"+str(year)+"_"+tileId+".tif")
		fPath =  "%s%s%s%s%s%s" % (r"01_data/", locationKey, "/tiles/", str(year), "/geoTiffs/", fName)

		bands_output = {"coordinates":[], "lstf":[], "ndvi":[], "rgb":[]}
		start_time = time.time()
		with rio.open(fPath) as src:
			print("opened: ", locationKey+"_"+str(year)+".tif")
				...
				for j in range(1,src_width-(resampling_size+1),resampling_size):
					...
					for k in range(len(tempRanges)):
						...
						if tempRange[0] < lstf < tempRange[1]:
							lstf_range = k
							break
					bands_output["lstf_range"][-1].append(lstf_range)'''
		...
		output_path = "%s%s%s%s%s%s" % (r"01_data/",locationKey,"/tiles/",str(year),"/json/",output_fname)
